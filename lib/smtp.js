...
async function handshakeWithRetries(email, mxHost, opts, onProgress, meta) {
  const maxRetries = opts.maxConnectionRetries ?? 2;
  const retryDelays = opts.retryDelaysMs ?? [10000, 30000];
  const baseTimeout = opts.connectTimeoutMs ?? 8000;

  let lastResult = await handshakeOnHost(email, mxHost, {
    ...opts,
    connectTimeoutMs: baseTimeout,
  });

  for (let attempt = 1; attempt <= maxRetries && isConnectionError(lastResult); attempt++) {
    const waitMs = retryDelays[attempt - 1] ?? retryDelays[retryDelays.length - 1];
    emitProgress(onProgress, {
      stage: 'retry',
      email: meta.email,
      domain: meta.domain,
      mxHost,
      mxIndex: meta.mxIndex,
      mxTotal: meta.mxTotal,
      attempt,
      maxAttempts: maxRetries + 1,
      waitMs,
    });
    await sleep(waitMs);
    lastResult = await handshakeOnHost(email, mxHost, {
      ...opts,
      connectTimeoutMs: baseTimeout + attempt * 5000,
    });
  }
...
async function verifyEmail(rawEmail, options = {}, onProgress) {
  ...
  for (let mxIndex = 0; mxIndex < mxHosts.length; mxIndex++) {
    ...
    lastResult = await handshakeWithRetries(parsed.email, mxHost, opts, onProgress, {
      email: parsed.email,
      domain: parsed.domain,
      mxIndex,
      mxTotal: mxHosts.length,
...
function buildMeaning(result) {
  ...
  if (result.error) {
    if (isConnectionError(result)) {
      return `${result.error} — mail server may be rate-limiting this IP; try a longer delay or wait before retrying.`;
    }
    return result.error;
  }
